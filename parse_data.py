...
input_file = os.path.join(base_dir, "Dữ liệu thủy triều.md")
...
for line in lines:
    ...
    max1_time = parse_time(parts[h_idx])
    h_idx += 1
    max1_val = parse_val(parts[h_idx])
    h_idx += 1
    max2_time = parse_time(parts[h_idx])
    h_idx += 1
    max2_val = parse_val(parts[h_idx])
    h_idx += 1
    min1_time = parse_time(parts[h_idx])
    h_idx += 1
    min1_val = parse_val(parts[h_idx])
    h_idx += 1
    min2_time = parse_time(parts[h_idx])
    h_idx += 1
    min2_val = parse_val(parts[h_idx])
